...
export class APIClient {
  ...
  private async ensureAdminToken(forceRefresh: boolean = false): Promise<string | null> {
  ...
  private async request<T>(
    ...
    try {
      ...
      if ((response.status === 401 || response.status === 403) && endpoint.startsWith("/api/")) {
        const refreshedToken = await this.ensureAdminToken(true);
        if (refreshedToken) {
   ...
   */
  async getApprovedProducts(): Promise<ApprovedProductsResponse> {
    return this.request("/api/approved-products");
  }
