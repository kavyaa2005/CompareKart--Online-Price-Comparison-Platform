...
from database import (
    ...
    get_all_approved_products, approve_product, reject_product, is_product_approved
)
...
def compare_platforms(product_name: str):
    ...
    Returns:
    - Price comparison across all available platforms for the product
    """
    ...
    # Get latest data for each available platform
    comparison = {}
    available_platforms = sorted(filtered['platform'].unique().tolist())
...
def user_compare(product_name: str, user: dict = Depends(require_user_role)):
    ...
    if product_name not in approved_products:
        raise HTTPException(status_code=404, detail=f"Product '{product_name}' is not approved for comparison")
...
def user_get_products(user: dict = Depends(require_user_role)):
    ...
    if df_data is None:
        raise HTTPException(status_code=503, detail="Data not loaded")
    products = sorted(approved_products.keys())
...
# Load approved products from database on startup
approved_products: Dict[str, Dict[str, Any]] = get_all_approved_products()


def _build_matching_pairs(include_approved: bool = False) -> List[Dict[str, Any]]:
    """Build product matching pairs from current dataset."""
    if df_data is None:
        return []
    ...
    for product in products:
        if not include_approved and product in approved_products:
            continue
        ...
        for platA, platB in combos[:2]:
            ...
            confidence = 98.0
            confidence -= min(price_diff_pct * 0.3, 10)
            confidence -= min(rating_diff * 2, 5)
            confidence -= min(discount_diff * 0.1, 3)
            confidence = round(max(confidence, 72.0), 1)
...
def get_matching_pairs(user: dict = Depends(require_admin_role)):
    ...
    if df_data is None:
        raise HTTPException(status_code=503, detail="Data not loaded")
    pairs = _build_matching_pairs(include_approved=False)
...
def get_matching_stats(user: dict = Depends(require_admin_role)):
    ...
    if df_data is not None:
        ...
        pending = len(_build_matching_pairs(include_approved=False))
    else:
        possible_pairs = 0
        pending = 0
...
@app.get("/api/approved-products")
def get_approved_products(user: dict = Depends(require_admin_role)):
    """Get products approved from matching workflow."""
    approved = sorted(approved_products.keys())
    return {"products": approved, "count": len(approved)}
...
def matching_action(pair_id: int, action: str, user: dict = Depends(require_admin_role)):
    ...
    product_name = pair["productA"]["name"]
    ...
    if action == "approved":
        # Save to database
        approve_product(product_name, pair_id)
        # Update in-memory registry for fast access
