      ...
      const hydrated = await Promise.all(
        productNames.map(async (productName, index) => {
          try {
            ...
            const changeDirection = prediction.price_change_percentage < 0 ? 'drop' : 'rise';
            const aiReason =
              changeDirection === 'drop'
                ? `AI expects ~${Math.abs(prediction.price_change_percentage).toFixed(1)}% price drop soon.`
                : `AI expects ~${Math.abs(prediction.price_change_percentage).toFixed(1)}% price rise.`;
            ...
            const product: Product = {
              ...
              recommendation: recommendationFromApi(prediction.recommendation),
              confidence: toConfidencePercent(prediction.confidence),
              aiReason,
              platforms: platformRows,
              priceHistory: trend.history.map((point) => ({ date: point.date, price: point.price })),
              lowestPrice: trend.min_price,
              highestPrice: trend.max_price,
            };
